...
export default function App() {
  ...
  return (
    <div className={dark ? "dark bg-black text-white min-h-screen" : "bg-white text-black min-h-screen"}>
      <button
        onClick={toggleTheme}
        className="fixed top-4 right-4 z-50 p-2 bg-gray-200 dark:bg-gray-800 rounded-full shadow-md transition-colors"
        aria-label="Toggle Theme"
      >
        {dark ? <Sun className="w-5 h-5 text-yellow-400" /> : <Moon className="w-5 h-5 text-gray-900" />}
      </button>
